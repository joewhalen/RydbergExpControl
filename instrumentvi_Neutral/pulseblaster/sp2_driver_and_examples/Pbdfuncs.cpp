...
int set_freq(double freq)
{
	...
	unsigned int freq_word = (unsigned int) (freq * pow(2,32) / (clock*1000)); // Desired data to trasnfer
...
int set_phase(double phase)
{
	...
	unsigned int phase_word = (int) (phase * pow(2,32) / 360); // Desired data to transfer
	unsigned int temp_byte;
	int return_value;
...
*/
	...
	double temp_delay = ((length * clock) - 3); //(Assumes clock in GHz and length in ns)
	
	if (temp_delay < 2 || temp_delay > (pow(2,32) - 1)) // Outside Valid Range
	{
		return PARAM_ERROR;
	}
